...
export const ContextGuide: React.FC<ContextGuideProps> = ({ title, text, items }) => {
  return (
    <motion.div 
      ...
      className="flex flex-col gap-3 rounded-2xl border border-[#2195d3]/20 bg-[#2195d3]/5 p-4"
    >
      <div className="flex items-center gap-3">
        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#2195d3]/20 text-[#2195d3]">
          <Info size={16} />
        </div>
        ...
        <div className="grid gap-3 pt-2 border-t border-white/5">
          {items.map((item, i) => (
            <div key={i} className="flex flex-col gap-0.5">
              <div className="text-[11px] font-bold uppercase tracking-wider text-[#2195d3]">
                {item.title}
              </div>
