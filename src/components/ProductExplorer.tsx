    ...
    return (
      <div className="flex flex-1 flex-col px-6 pt-4 pb-24">
        <div className="flex items-center justify-between mb-6">
          <div className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#2195d3]">
            SOLUTION PREVIEW
          </div>
          <button 
        ...
        <div className="flex flex-col gap-6">
          <div className="aspect-[16/10] w-full rounded-2xl bg-[#0d4f73] border border-white/5 flex items-center justify-center">
    ...
    <div className="flex flex-1 flex-col px-6 pt-4 pb-24">
      <div className="flex flex-col gap-0">
        ...
        <ScreenHeadline
          ...
          name={firstName}
          subtext={screen.content.subtext || "Select one or more."}
        />
      </div>
          ...
          return (
            <button
              ...
              className={`flex flex-col rounded-2xl border p-4 transition-all duration-300 ${
                isSelected 
                  ? "border-[#2195d3] bg-[#2195d3]/10" 
                  : "border-white/5 bg-white/[0.03] hover:border-white/10 hover:bg-white/[0.05]"
              }`}
            >
              <div className={`flex h-9 w-9 items-center justify-center rounded-lg transition-colors ${
                isSelected ? "bg-[#2195d3] text-white" : "bg-white/5 text-white/40"
              }`}>
              ...
              <div className="mt-4 flex flex-col gap-1.5">
                ...
                {opt.proof && (
                  <div className="mt-2 text-[10px] font-medium uppercase leading-[1.4] tracking-[0.07em] text-[#2195d3]">
                    {opt.proof}
                  </div>
                )}
              </div>
        ...
        {flow.selectedProductIds.length > 0 ? (
          ...
          >
            {flow.selectedProductIds.length > 1 
              ? "Check me in →"
              : (screen.content.options?.find(o => o.id === flow.selectedProductIds[0])?.cta_label || "Check me in →")
            }
          </PrecisionButton>
        ) : (
          <button 
            className="w-full text-center text-[11px] font-bold uppercase tracking-widest text-[#2195d3] hover:text-[#2195d3]/80 transition-colors py-4" 
            onClick={onFinish}
          >
