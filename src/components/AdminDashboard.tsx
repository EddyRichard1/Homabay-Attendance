...
function statusClassName(status: string) {
  if (status === "checked_in") return "text-emerald-700";
  if (status === "follow_up") return "text-amber-600";
      ...
      <SignedIn>
        {isAdmin ? (
          ...
          <div className="flex min-h-[100dvh] flex-col items-center justify-center bg-[var(--color-cs-light-bg)] px-6 text-center">
            <h1 className="text-[24px] font-bold text-[#0a1628]">Access Denied</h1>
            <p className="mt-2 text-[14px] text-[#0a1628]/60 max-w-[320px]">
              This dashboard is restricted to authorized administrative personnel only.
            </p>
            
            <div className="mt-8 rounded-[12px] border border-[#0a1628]/5 bg-white p-6 shadow-sm">
              <h2 className="text-[12px] font-semibold uppercase tracking-wider text-[#0a1628]/40">Diagnostic Info</h2>
              <div className="mt-4 flex flex-col gap-2 text-left">
                <div className="flex justify-between gap-8 text-[13px]">
                  <span className="text-[#0a1628]/40 text-nowrap">Your Email:</span>
                  <span className="font-mono text-[#0a1628] truncate max-w-[140px]">{userEmail}</span>
                </div>
                <div className="flex justify-between gap-8 text-[13px]">
                  <span className="text-[#0a1628]/40 text-nowrap">Current Role:</span>
                  <span className={`font-medium ${role ? "text-[var(--color-cs-blue)]" : "text-amber-500"}`}>
                    {role || "None detected"}
                  </span>
                </div>
              </div>
              <p className="mt-6 text-[11px] text-[#0a1628]/40 max-w-[280px]">
              ...
              <button 
                ...
                className="mt-6 w-full rounded-[8px] border border-[#0a1628]/10 py-[8px] text-[12px] font-medium text-[#0a1628]/60 hover:bg-[#F7F9FC]"
              >
                Refresh Session
              </button>
            </div>
...
function AdminDashboard() {
  ...
  const attendees = leads.filter((lead) => lead.eventId === CONFIG.EVENT_ID);

  const filteredLeads = attendees.filter(
    (lead) =>
  ...
  const exportCSV = async () => {
    const headers = ["Name", "Organisation", "Role", "Email", "Phone", "Affiliation", "Status", "Time", "Date"];
    const rows = attendees.map((lead) => {
      ...
      return [
        ...
        `"${lead.email}"`,
        `"${lead.phone}"`,
        `"${affiliationLabel(lead.productInterest)}"`,
        `"${lead.status}"`,
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--color-cs-light-bg)] pb-12">
      {/* Header */}
      <header className="sticky top-0 z-40 flex items-center justify-between bg-[var(--color-cs-navy-surface)] px-4 py-3 md:px-8 md:py-4">
        <div className="flex items-baseline gap-3">
          <h1 className="text-[14px] font-semibold text-white md:text-[16px]">Launch Attendance</h1>
          <span className="hidden text-[12px] text-white/70 sm:inline">
            {CONFIG.EVENT_NAME} · {stats.total} total
          </span>
        </div>
      ...
      {/* QR Code Modal */}
      {showQR && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0a1628]/90 p-4 backdrop-blur-sm">
          <div className="relative w-full max-w-[400px] rounded-2xl bg-white p-6 shadow-2xl md:p-8">
            <button
              onClick={() => setShowQR(false)}
              className="absolute right-4 top-4 text-[#0a1628]/40 hover:text-[#0a1628]"
            >
              <X size={20} />
            </button>
      ...
      <main className="mx-auto mt-6 w-full max-w-[1200px] px-4 md:mt-8 md:px-8">
        ...
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          <div className="rounded-xl border border-[var(--color-cs-light-border)] bg-white p-4 shadow-sm md:p-6 text-center md:text-left">
            <span className="text-[10px] uppercase tracking-wider text-[#0a1628]/40">Total attendees</span>
            <div className="mt-1 flex items-baseline justify-center md:justify-start gap-2">
              <span className="text-2xl font-bold text-[#0a1628] md:text-3xl">{stats.total}</span>
              <Building2 className="text-[var(--color-cs-blue)]/20" size={16} />
            </div>
          </div>
          <div className="rounded-xl border border-[var(--color-cs-light-border)] bg-white p-4 shadow-sm md:p-6 text-center md:text-left">
            <span className="text-[10px] uppercase tracking-wider text-[#0a1628]/40">Today</span>
            <div className="mt-1 flex items-baseline justify-center md:justify-start gap-2">
              <span className="text-2xl font-bold text-[#0a1628] md:text-3xl">{stats.today}</span>
        ...
        <div className="mt-8 flex flex-col gap-4 md:flex-row md:items-center">
          <div className="relative flex-1">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#0a1628]/30" size={16} />
            <input
              type="text"
              placeholder="Search attendees..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full rounded-xl border border-[#0a1628]/10 bg-white py-3 pl-10 pr-4 text-[14px] text-[#0a1628] outline-none transition-colors focus:border-[var(--color-cs-blue)]"
            />
          </div>
        </div>
            ...
            return (
              <div key={lead._id} className="rounded-2xl border border-[var(--color-cs-light-border)] bg-white p-5 shadow-sm">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <h3 className="font-bold text-[#0a1628]">{lead.name}</h3>
                    <div className="flex flex-col gap-0.5 mt-1 text-[12px] text-[#0a1628]/50">
                  ...
                  <select 
                    ...
                    className={`rounded-lg border-none bg-[var(--color-cs-light-bg)] px-2 py-1 text-[11px] font-bold outline-none ring-1 ring-black/5 ${statusClassName(lead.status)}`}
                  >
                    {!ATTENDANCE_STATUSES.some((s) => s.value === lead.status) && (
                ...
                <div className="space-y-2 mt-4 pt-4 border-t border-[var(--color-cs-light-bg)]">
                  <div className="flex items-center gap-2 text-[13px] text-[#0a1628]/60">
                    <Mail size={14} className="text-[var(--color-cs-blue)]/40" />
                    <span className="truncate">{lead.email}</span>
                  </div>
                  <div className="flex items-center gap-2 text-[13px] text-[#0a1628]/60">
                    <Globe size={14} className="text-[var(--color-cs-blue)]/40" />
                    <span className="rounded-full bg-sky-50 px-2.5 py-0.5 text-[10px] font-bold text-[var(--color-cs-blue)]">
                      {affiliationLabel(lead.productInterest)}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 text-[12px] text-[#0a1628]/30 mt-2">
                    <Clock size={12} />
          ...
          <table className="w-full text-left">
            <thead>
              <tr className="bg-[var(--color-cs-light-bg)]/50">
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40">Name</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40">Organisation</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40">Affiliation</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40">Status</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40">Contact</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-wider text-[#0a1628]/40 text-right">Time</th>
              </tr>
            </thead>
                ...
                return (
                  <tr key={lead._id} className="group hover:bg-[var(--color-cs-light-bg)]/30 transition-colors">
                    <td className="px-6 py-4">
                      <span className="text-[14px] font-bold text-[#0a1628]">{lead.name}</span>
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-[14px] text-[#0a1628]/70">{lead.organisation}</span>
                        <span className="text-[11px] text-[#0a1628]/40 italic">{(lead as any).role}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                      <span className="inline-flex rounded-full bg-sky-50 px-3 py-1 text-[11px] font-bold text-[var(--color-cs-blue)] ring-1 ring-sky-100">
                        {affiliationLabel(lead.productInterest)}
                      </span>
                    </td>
                    <td className="px-6 py-4">
                      <select 
                        value={lead.status}
                        onChange={(e) => updateStatus({ id: lead._id, status: e.target.value })}
                        className={`rounded-lg border-none bg-transparent text-[13px] font-bold outline-none focus:ring-2 focus:ring-[var(--color-cs-blue)]/20 ${statusClassName(lead.status)}`}
                      >
                        {!ATTENDANCE_STATUSES.some((s) => s.value === lead.status) && (
                          <option value={lead.status}>{lead.status}</option>
                        )}
                    ...
                    <td className="px-6 py-4">
                      <div className="flex flex-col">
                        <span className="text-[13px] text-[#0a1628]">{lead.email}</span>
                        <span className="text-[11px] text-[#0a1628]/30">{lead.phone}</span>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="text-[12px] text-[#0a1628]/40">{d.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</span>
                    </td>
                  </tr>
                );
              })}
        ...
        {filteredLeads.length === 0 && (
          <div className="mt-12 text-center text-[#0a1628]/40">
